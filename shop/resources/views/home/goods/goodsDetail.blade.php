		...
		<section class="item-show__div item-show__head clearfix">
			<div class="pull-left">
				<ol class="breadcrumb">
					...
					<li><a href="item_sale_page.html">{{$rescate}}</a></li>
					<li><a href="item_sale_page.html">{{$rescate1}}</a></li>
					<li><a href="item_sale_page.html">{{$rescate2}}</a></li>
					<li class="active">{{$result->goods_name}}</li>
				</ol>
				<div class="item-pic__box" id="magnifier">
					<div class="small-box">
						<img class="cover" src="{{URL('').$result->goods_photo}}" alt="{{$result->goods_name}}">
					...
					<div class="thumbnail-box">
						...
						<div class="thumb-list">
							<ul class="wrapper clearfix">
								...
								<li class="item" data-src="{{URL('').$v->goods_picurl}}"><img class="cover" src="{{URL('').$v->goods_picurl}}" alt="商品预览图"></li>
							

								@endforeach
								<script>
									$('#magnifier .thumbnail-box .thumb-list li').eq(0).addClass('active');
								</script>
					...
					<div class="big-box"><img  src="{{URL('').$result->goods_photo}}" alt="{{$result->goods_name}}"></div>
				</div>
				<script src="{{asset('home/js/jquery.magnifier.js')}}"></script>
				<script>
					$(function () {
						$('#magnifier').magnifier();
					});
				...
				<div class="item-info__box">
					<div class="item-title">
						<div class="name ep2">{{$result->goods_name}}</div>
						<div class="sale cr">优惠活动：该商品享受8折优惠</div>
					</div>
					<div class="item-price bgf5">
						<div class="price-box clearfix">
							<div class="price-panel pull-left">
								售价：<span class="price">￥{{$result->goods_price}} <s class="fz16 c9">￥24.00</s></span>
							</div>
					...
					<ul class="item-ind-panel clearfix">
						<li class="item-ind-item">
							...
							<span class="ind-count cr">{{$result->goods_sales}}</span>
						</li>
						<li class="item-ind-item">
							<a href=""><span class="ind-label c9">累计评论</span>
							<span class="ind-count cr">{{count($comment)}}</span></a>
						</li>
						<li class="item-ind-item">
					...
					<div class="item-key">
						...
						<div class="item-amount clearfix bgf5">
							...
							<div class="amount-box">
								<div class="amount-widget">
									<input class="amount-input" value="1" id="goods_num" maxlength="8" title="请输入购买量" type="text">
									<div class="amount-btn">
										<a class="amount-but add"></a>
										<a class="amount-but sub"></a>
									</div>
								</div>
								<div class="item-stock"><span style="margin-left: 10px;">库存 <b id="Stock">{{$result->goods_inventory}}</b> 件</span></div>
								<script>
									$(function () {
						...
						<div class="item-action clearfix bgf5">
							<a href="javascript:;" rel="nofollow" data-addfastbuy="true" title="点击此按钮，到下一步确认购买信息。" role="button" id='item-action__buy' class="item-action__buy">立即购买</a>
			...
			<script>
				...
				$('.item-action__basket').click(function(){
					...
					$.post("{{URL('home/cart/cartadd')}}",{'goods_num':goods_num,'goods_id':{{$result->goods_id}},'_token': "{{ csrf_token() }}"},function(r){
						//console.log(r);
							//alert(r);
							if(r==1){
								layer.msg('已添加至购物车',{icon: 6,time:1000});
							}else{
								layer.msg('添加购物车失败',{icon: 5,time:2000});
							}
					});

				});

				//点击立刻购买后的ajax提交，此时把当前产品项在购物车中状态变为1
				$('.item-action__buy').click(function(){
					//alert(1);
					//获得当前的购买数量
					goods_num=$('#goods_num').val();
					$.post("{{URL('home/cart/cartaddnow')}}",{'goods_num':goods_num,'goods_id':{{$result->goods_id}},'_token': "{{ csrf_token() }}"},function(r){
						//console.log(r);
							//alert(r);
							if(r==1){//如果处理数据添加成功到购物车，则跳转到购物车页面
								window.location.href="{{URL('home/cart/cartlist')}}";
							}else{
								layer.msg('发生错误',{icon: 5,time:2000});
							}
					});
		...
		<section class="item-show__div item-show__body posr clearfix">
			<div class="item-nav-tabs">
				<ul class="nav-tabs nav-pills clearfix" role="tablist" id="item-tabs">
					...
					<li role="presentation"><a href="#evaluate" role="tab" data-toggle="tab" aria-controls="evaluate">累计评价 <span class="badge">{{$comment->total()}}</span></a></li>
					...
					<div role="tabpanel" class="tab-pane fade" id="evaluate" aria-labelledby="evaluate-tab">
						<div class="evaluate-tabs bgf5">
							<ul class="nav-tabs nav-pills clearfix" role="tablist">
								<li role="presentation" class="active"><a href="#all" role="tab" data-toggle="tab" aria-controls="all" aria-expanded="true">全部评价 <span class="badge">{{$comment->total()}}</span></a></li>
								<li role="presentation"><a href="#good" role="tab" data-toggle="tab" aria-controls="good">好评 <span class="badge">{{$Gcomment->total()}}</span></a></li>
								<li role="presentation"><a href="#normal" role="tab" data-toggle="tab" aria-controls="normal">中评 <span class="badge">{{$Mcomment->total()}}</span></a></li>
								<li role="presentation"><a href="#bad" role="tab" data-toggle="tab" aria-controls="bad">差评 <span class="badge">{{$Ncomment->total()}}</span></a></li>
							</ul>
						</div>
								...
								<div role="tabpanel" class="tab-pane fade in active" id="all" aria-labelledby="all-tab">
									...
									<div class="eval-box">
										<div class="eval-author">
											<div class="port">
												<img src="{{URL('home/images/icons/default_avt.png')}}" alt="欢迎来到U袋网" class="cover b-r50">
											</div>
											<div class="name">
											@if($v->anonym==1)
												{{$v->user_id}}
 ...
 -->
					...
					<style>
						...
						<a class="disabled">1/60页</a> -->
							...
							<script src="{{asset('home/js/jquery.zoom.js')}}"></script>
						</div>
					</div>
