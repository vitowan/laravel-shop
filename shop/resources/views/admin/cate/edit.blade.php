...
@endif
	<form class="form-horizontal" role="form" action="{{ URL('admin/cate').'/'.$data->id }}" method='post'>
		{{csrf_field()}}
		...
		<div class="form-group" style="margin-top:50px">
			...
			<div class="col-sm-9">
				<input type="text" name='cate_name' value="{{$data->cate_name}}" id="cate_name" class="col-xs-10 col-sm-5">
			</div>
		</div>
		...
		<div class="form-group" style="margin-top:50px">
			...
			<div class="col-sm-9">
				<select name="cate_pid" id="group_id" class="col-xs-10 col-sm-5">
					...
					@foreach($result as $v)
					<option value="{{$v->id}}" @if($data->cate_pid==$v->id) selected @else @endif>{{$v->cate_strname}}</option>
					@endforeach
				</select>
			</div>
		</div>	
