...
@endif
	<form class="form-horizontal" role="form" action="{{ URL('admin/user').'/'.$result->id }}" method='post' enctype='multipart/form-data'>
		{{csrf_field()}}
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<input type="text" name='username' value="{{$result->username}}" id="username" class="col-xs-10 col-sm-5">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<input type="password" name='password' value="{{$result->password}}" id="password"  class="col-xs-10 col-sm-5">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<label class="radio-inline">
				  <input type="radio" name="sex" @if($result->sex==1) checked @else @endif value="1"> 男
				</label>
				&nbsp;&nbsp;&nbsp;
				<label class="radio-inline">
				  <input type="radio" name="sex" @if($result->sex==0) checked @else @endif value="0"> 女
				</label>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<input type="text" name='phone' value="{{$result->phone}}" id="phone" class="col-xs-10 col-sm-5">
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-9">
				<input type="email" name='email' value="{{$result->email}}" id="email" class="col-xs-10 col-sm-5">
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-9">
				<select name="role_id" id="role_id" class="col-xs-10 col-sm-5">
					@foreach($role as $v)
					<option value="{{$v->role_id}}" @if($v->role_id==$role_id) selected @endif>{{$v->role_name}}</option>
					@endforeach
				</select>
			</div>
		</div>		
		...
		<div class="form-group">
			...
			<div class="row">
		    	<div class="col-sm-5 col-md-2" >
		        	 <div class="thumbnail" >
		           		 <img id='img_upload' style="width:150px;height:140px" src="{{$result->pic}}" 
		             alt="个人头像">
		           		 <div class="caption">
		                <p>
